...
router.use((req, res, next) => {
  console.log(`[HomeCleaningService] ${req.method} ${req.path}`, req.body);
  next();
});
...
// GET route to fetch all home cleaning services (for customers)
router.get("/", async (req, res) => {
  try {
    console.log('Fetching all home cleaning services');
    const services = await HomeCleaningService.find();
...
// GET route to fetch home cleaning services for a specific provider
router.get("/provider/:providerEmail", async (req, res) => {
  ...
  try {
    const services = await HomeCleaningService.find({ providerEmail });
...
router.post("/", async (req, res) => {
  ...
  try {
    const service = new HomeCleaningService({
      name,
      price: Number(price),
      time,
      description,
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const service = await HomeCleaningService.findById(id);
    if (!service) {
