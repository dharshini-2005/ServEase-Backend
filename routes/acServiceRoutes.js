...
router.use((req, res, next) => {
  console.log(`[ACService] ${req.method} ${req.path}`, req.body);
  next();
});

// GET route to fetch all AC services (for customers)
router.get("/", async (req, res) => {
  try {
    console.log('Fetching all AC services');
...
// GET route to fetch AC services for a specific provider
router.get("/provider/:providerEmail", async (req, res) => {
  ...
  try {
    ...
    const services = await ACService.find({ providerEmail });
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const service = await ACService.findById(id);
    if (!service) {
