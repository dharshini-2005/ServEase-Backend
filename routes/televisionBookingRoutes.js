...
const TelevisionBooking = require("../models/TelevisionBooking");
const TelevisionService = require("../models/TelevisionService");
const router = express.Router();

// Debug middleware for this router
router.use((req, res, next) => {
  console.log(`[TelevisionBooking] ${req.method} ${req.path}`, req.body);
  next();
});
...
router.get("/customer/:customerEmail", async (req, res) => {
  ...
  try {
    const bookings = await TelevisionBooking.find({ customerEmail })
      .populate('serviceId')
      .sort({ bookingDate: -1 });
...
router.get("/provider/:providerEmail", async (req, res) => {
  ...
  try {
    ...
    const bookings = await TelevisionBooking.find({ serviceId: { $in: serviceIds } })
      .populate('serviceId')
      .sort({ bookingDate: -1 });
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const service = await TelevisionService.findById(serviceId);
    if (!service) {
      return res.status(404).json({ message: "Service not found" });
    }

    const booking = new TelevisionBooking({
      serviceId,
      customerEmail,
      status: 'pending',
...
router.put("/:id/status", async (req, res) => {
  ...
  try {
    const booking = await TelevisionBooking.findById(id);
    if (!booking) {
