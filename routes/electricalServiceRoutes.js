...
router.use((req, res, next) => {
  console.log(`[ElectricalService] ${req.method} ${req.path}`, req.body);
  next();
});

// GET route to fetch all electrical services (for customers)
router.get("/", async (req, res) => {
  try {
    console.log('Fetching all electrical services');
...
// GET route to fetch electrical services for a specific provider
...
router.post("/", async (req, res) => {
  ...
  try {
    const service = new ElectricalService({
      name,
      price: Number(price),
      time,
      description,
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const service = await ElectricalService.findById(id);
    if (!service) {
