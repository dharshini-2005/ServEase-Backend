...
router.use((req, res, next) => {
  console.log(`[TelevisionService] ${req.method} ${req.path}`, req.body);
  next();
});

// GET route to fetch all television services (for customers)
router.get("/", async (req, res) => {
  try {
    console.log('Fetching all television services');
    const services = await TelevisionService.find();
...
// GET route to fetch television services for a specific provider
router.get("/provider/:providerEmail", async (req, res) => {
  ...
  try {
    console.log('Attempting to find services with query:', { providerEmail });
    const services = await TelevisionService.find({ providerEmail });
...
router.post("/", async (req, res) => {
  ...
  try {
    const service = new TelevisionService({
      name,
      price: Number(price),
      time,
      description,
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const service = await TelevisionService.findById(id);
    if (!service) {
